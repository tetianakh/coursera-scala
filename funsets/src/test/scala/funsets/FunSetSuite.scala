...
class FunSetSuite extends FunSuite {
  ...
  test("forall should check if all elements satisfy requirement") {
    new TestSets {
      val s12 = union(s1, s2)
      assert(forall(s12, x => x > 0), "forall 1")
      assert(!forall(s12, x => x < 0), "forall 2")
    }
  }

  test("exist should check if such elements exists in set") {
    new TestSets {
      val s12 = union(s1, s2)
  ...
  test("map should transform set elements") {
    new TestSets {
      val s = map(union(s1, s2), x=> x*x)
      assert(contains(s, 4), "Should square the elements")
    }
  }
}
